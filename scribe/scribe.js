...
function startAskMe(question, reponses, options, callback) {
    SarahEcoute(false, function() {
        _ScribeSpeak(question, function() {
            ...
            token_askme = setTimeout(function() {
                __hookScribe(TIME_ELAPSED);
            }, options.timeout)
        })
    });
}
    ...
    if (callback != false) {
        for (i = 0; i < txt.length; i++) {
            ...
            if (i != txt.length - 1) {
                ...
                open_p2 = txt[i + 1].indexOf("(");
                if (open_p2 == -1) open_p2 = 9999;
                close_p2 = txt[i + 1].indexOf(")"); //if (close_p2==-1) close_p2=99999;
                par2 = (open_p2 > close_p2);
                if (par && par2) parenthese = true;
                else parenthese = false;
...
function sloop(i) {
    if (typeof say[i] !== 'undefined') {
        if (say[i].last == false || (say[i].last == true && i == say.length - 1)) {
            ...
            if (i < say.length) {
                sloop_token = setTimeout(function() {
                    sloop(i);
                }, say[i].timeout);
            }
        } else {
...
function scribe_speak(tts, async, SARAH) {
    ...
    tts_queue.push({
        'tts': tts,
        'callback': (async ? async : undefined)
    });
...
exports.action = function(data, callback) {
    ...
    if (typeof data.reco !== 'undefined') {
        console.log("SCRIBE CALL LOG: " + util.inspect(data, {
            showHidden: true,
            depth: null
        }));
        ...
        SARAH.context.scribe.lastX.unshift({
            'compteur': SARAH.context.scribe.compteur,
            'reco': data.reco,
...
function autorun_browser(kill) {
    ...
    var child = exec(proc, function(error, stdout, stderr) {
        ...
        if (kill == true) setTimeout(function() {
