...
exports.init = function(SARAH) {
    ...
    loc.addDictEntry("S1", function() {
        return g_s1;
    }, function(val) {
        g_s1 = val;
    });
    loc.addDictEntry("S2", function() {
        return g_s2;
    }, function(val) {
        g_s2 = val;
    });
    loc.addDictEntry("CHOICE", function() {
        return g_choice;
    }, function(val) {
        g_choice = val;
    });
    ...
    g_choice = chooseSign();
    info('[ Chifoumi ] is initializing ...');
}

exports.action = function(data, callback) {

    SARAH.context.scribe.activePlugin('Chifoumi');
    ...
    switch (data.choice) {
        ...
        default:
            ...
            if (res == 0) {
                txt = loc.getLocalString("RESULT1");
            }
            if (res == -1) {
                g_s1++;
                txt = loc.getLocalString("RESULT2");
            }
            if (res == 1) {
                g_s2++;
                txt = loc.getLocalString("RESULT3");
            }
